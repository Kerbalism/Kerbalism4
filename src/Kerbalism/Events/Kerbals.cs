...
namespace KERBALISM.Events
{
	...
	class KerbalEVA_BoardPart
	{
		static bool Prefix(KerbalEVA __instance, Part p)
		{
			...
			return GameEventsKerbals.AttemptBoard(__instance, p);
		}
	}


	public class GameEventsKerbals
	{
		...
		public void ToEVA(GameEvents.FromToAction<Part, Part> data)
		{
			...
			SupplyDefinition.SetupEva(data.to);
			...
			{
				// get the resource
				VesselResourceKSP evaRes = evaVD.ResHandler.GetKSPResource(partRes.info.id);
				VesselResourceKSP vesselRes = vesselVD.ResHandler.GetKSPResource(partRes.info.id);
		...
		public void FromEVA(GameEvents.FromToAction<Part, Part> data)
		{
			...
			{
				vesselVD.ResHandler.GetKSPResource(partRes.info.id).Produce(partRes.amount);
			}

			// merge drives data
			DriveHandler.MoveAllFromVesselToVessel(evaVD, vesselVD);
