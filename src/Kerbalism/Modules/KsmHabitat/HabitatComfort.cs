...
namespace KERBALISM
{
	public class ComfortDefinition
	{
		...
		public static Dictionary<string, ComfortDefinition> definitions = new Dictionary<string, ComfortDefinition>();

		public const string notAlone = "notAlone";
		public const string callHome = "callHome";
		public const string firmGround = "firmGround";
		...
		public static void Load(ConfigNode comfortNode)
		{
			ComfortDefinition instance = new ComfortDefinition();
			CFGValue.Parse(instance, comfortNode);

			if (string.IsNullOrEmpty(instance.name) || definitions.ContainsKey(instance.name))
			{
				ErrorManager.AddError(false, $"Empty or duplicate {Profile.NODENAME_COMFORT} definition with name : {instance.name}");
				return;
			}

			definitions.Add(instance.name, instance);
		}
		...
		public ComfortInfoBase GetComfortInfo()
		{
			switch (name)
			{
				case notAlone: return new ComfortNotAlone(this);
				case callHome: return new ComfortCallHome(this);
				case firmGround: return new ComfortFirmGround(this);
				default: return new ComfortModuleInfo(this);
			}
		}
	}
	...
	public class ComfortValue
	{
		...
		{
			ComfortValue comfort = new ComfortValue();
			string name = Lib.ConfigValue(comfortNode, "name", string.Empty);
			if (!ComfortDefinition.definitions.TryGetValue(name, out ComfortDefinition definition))
			{
				ErrorManager.AddError(false, $"Error parsing COMFORT node, no definition for a `{name}` comfort");
				return null;
			}

			comfort.Definition = definition;
			CFGValue.Parse(comfort, comfortNode);
			return comfort;
		}
	}
