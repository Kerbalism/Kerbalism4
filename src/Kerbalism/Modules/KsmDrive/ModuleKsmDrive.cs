...
namespace KERBALISM
{
		...
		IPartCostModifier
	{
		...
		public override void OnLoad(ConfigNode node)
		{
			...
			moduleHandler = new DriveHandler();
			moduleHandler.dataCapacity = effectiveDataCapacity;
			moduleHandler.sampleCapacity = effectiveSampleCapacity;
		}
		...
		public void Update()
		{
			...
			if (Lib.IsFlight)
			{
				...
				Events["ToggleUI"].guiName = Lib.StatusToggle(Local.HardDrive_Data, moduleHandler.Empty() ? Local.HardDrive_Dataempty : moduleHandler.Size());//"Data""empty"
				Events["ToggleUI"].active = !IsPrivate();

				// show TakeData eva action button, if there is something to take
				Events["TakeData"].active = !moduleHandler.Empty();
				...
				if(transferVisible)
				{
					transferVisible = DriveHandler.GetDrives((VesselData)moduleHandler.VesselData, true).Count() > 1;
				}
		...
		{
			return moduleHandler.isPrivate;
		}
		...
		private void UpdateCapacity()
		{
			...
			double mass = 0;
			foreach (var sample in moduleHandler.samples.Values) mass += sample.mass;
			...
			if (Lib.IsFlight)
			{
				availableDataCapacity = moduleHandler.FileCapacityAvailable();
				availableSlots = Lib.SampleSizeToSlots(moduleHandler.SampleCapacityAvailable());
			}
		...
		public void TransferData()
		{
			...
			foreach(var hardDrive in hardDrives)
			{
				...
				hardDrive.moduleHandler.Move(moduleHandler, PreferencesScience.Instance.sampleTransfer || Lib.CrewCount(vessel) > 0);
			}
		}
		...
		public void TakeData()
		{
			...
			// transfer data
			if(!DriveHandler.Transfer(moduleHandler, vd, PreferencesScience.Instance.sampleTransfer || Lib.CrewCount(v) > 0))
			{
				Message.Post
				(
		...
		public void StoreData()
		{
			...
			// transfer data
			if (!DriveHandler.Transfer(vd, moduleHandler, PreferencesScience.Instance.sampleTransfer || Lib.CrewCount(v) > 0))
			{
				Message.Post
				(
					Lib.Color(Lib.BuildString(Local.HardDrive_WARNING_title), Lib.Kolor.Red, true),//"WARNING: not evering copied"
		...
		public void ReturnData(ScienceData data)
		{
			...
			{
				moduleHandler.RecordFile(subjectData, data.dataAmount);
			}
			else
			{
				moduleHandler.RecordSample(subjectData, data.dataAmount, subjectData.ExpInfo.MassPerMB * data.dataAmount);
			}
		}
		...
		public void DumpData(ScienceData data)
		{
			...
			{
				moduleHandler.DeleteFile(subjectData, data.dataAmount);
			}
			else
			{
				moduleHandler.DeleteSample(subjectData, data.dataAmount);
			}
		}
